...
class Attn_Net(nn.Module):    # 维度变化  L-->D-->n_classes
...
class Attn_Net_Gated(nn.Module):   #  注意力是否使用sigmoid gating   参考LSTM
    def __init__(self, L=1024, D=256, dropout=False, n_classes=1):  #  n_classes默认为1，就表示每个patch的重要性
    ...
    def forward(self, x):
        ...
        return A, x  # A代表注意力分数，即每个patch的重要性，x还是原来每个patch的信息
...
class CLAM_Batch(nn.Module):
    ...
    def forward(self, batch, return_global_feature=False):
        h, label = batch   # h: [b, n, d]
        
        bsz = h.shape[0]   # h.size(0)等效
        h = rearrange(h, 'b n d -> (b n) d')   # 方便后面的attention_net的线性层
        A, h = self.attention_net(h)  #   A　是注意力分数　　

        A = rearrange(A, '(b n) d -> b d n', b=bsz) # B x K x N, e.g., [32,n,1]－－> [32,1,n]
        h = rearrange(h, '(b n) d -> b n d', b=bsz) # 32*12000*128
        
        # this temperature control the shape to be sharp (< 1) or flat (>1);
        tau = 1.5 

        # A = torch.transpose(A, 2, 1)  # B x K x N
        A = F.softmax(A / tau, dim=-1)  # softmax over N
        
        if self.instance_eval:   # 是否加入实例损失  默认为False  这个是创新点  cluster

            total_inst_loss = 0.0
            all_preds = []
            all_targets = []

            for inst in range(bsz):   # bsz 是batch_size

                A_inst = A[inst]   # A_inst 单个bag的注意力分数了，而且是经过softmax后的   [1,n]  n是每个bag中的instance的数量
                h_inst = h[inst]
                label_inst = label[inst]

                # an ugly walkaround to deal with small bag size
                original_k_sample = self.k_sample  # k_sample 默认为16    我们取前k的最大和前k个最小
        ...
        M = torch.bmm(A, h).squeeze(1)  # 32*1024  bmm之后是 [b.1,d]  然后压缩 --> [b,d]

        logits = self.classifiers(M)  # logits不是概率，只是神经网络的原始输出
        Y_hat = torch.topk(logits, 1, dim=1)[1]   # (value,indices)   
        Y_prob = F.softmax(logits, dim=1)
        if self.instance_eval:  #   默认为False，不计算实例损失
            results_dict = {'instance_loss': total_inst_loss, 'inst_labels': np.array(all_targets),
                            'inst_preds': np.array(all_preds)}
        else:
            results_dict = {}
...
if __name__ == "__main__":
    ...
    net = CLAM_Batch()
    logits, Y_prob, Y_hat,result_dict = net(batch=(x,y))     # 能运行
