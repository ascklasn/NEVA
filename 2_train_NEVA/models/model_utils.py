...
def initialize_weights(model):
    for m in model.modules():
        if isinstance(m, nn.Linear):
            nn.init.xavier_normal_(m.weight)  # 使用 Xavier 正态分布（Xavier Normal Distribution）来初始化模块 m 的权重。
            # m.bias.data.zero_()

        elif isinstance(m, nn.BatchNorm1d):  # 一维的 BN，往往用于 FC之后  [N,C]
            nn.init.constant_(m.weight, 1)  # 使得 BN 的 weight 和 bias 固定为0和1
...
class CoxSurvLoss(nn.Module):
    ...
    def forward(self, hazards, time, c):
        '''
        # hazards: 模型输出的风险值 (log风险)
        # time: 事件发生时间或者观察时间
        # c: 事件是否发生 (1表示事件发生，0表示被截断)
        '''

        # This calculation credit to Travers Ching https://github.com/traversc/cox-nnet
        # Cox-nnet: An artificial neural network method for prognosis prediction of high-throughput omics data
        
        # Ensure hazards contain exactly one risk score per sample
        hazards = hazards.reshape(len(time), -1)
        if hazards.shape[1] != 1:
            raise ValueError(
                f"CoxSurvLoss expects a single risk value per sample, got shape {hazards.shape}. "
                "Check the model output dimension (e.g., set num_classes=1 for Cox regression tasks)."
            )
