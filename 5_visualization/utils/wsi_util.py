...
# -*- coding: utf-8 -*-
# **********************************
# Author: hcshen,kezhouyan,kuantian #
# Email:  hcshen@tencent.com        #
# **********************************
"""
module docstring.
"""
...
def get_wsi_mpp(slide):
    ...
    elif 'tiff.ImageDescription' in wsi_info:
        image_info = wsi_info['tiff.ImageDescription']
        key_vals = image_info.split('|')
        rx = re.compile(r'(?P<key>\w+)\s*=\s*(?P<value>.+)')

        for key_val in key_vals:
            if 'MPP' not in key_val:
                continue
            params = {
                match.group('key'): match.group('value')
                for match in rx.finditer(key_val)
            }
            mpp = round(float(params['MPP']), 4)
    else:
...
def get_foreground_mask_and_components_model_process(slide, mask_downsamples, manager, gpu_id):
    tissue_seg_dict = manager.dict()
    p = multiprocessing.Process(
        target=get_foreground_mask_and_components_model,
        args=(slide, mask_downsamples, tissue_seg_dict, gpu_id))
    p.start()
    p.join()
...
def get_foreground_mask_and_components_model(
        slide, mask_downsamples, tissue_seg_dict, gpu_id=0):
    ...
    model_path = "../wsi_tissue_segmentation/inference_models/tissue_seg_v8_mpp16.0.pb"

    tf_model = TfModel(model_path, gpu_id=gpu_id)
    img_thumb = get_thumbnail_with_target_mpp(slide, target_mpp=8.0)
    _, thumb_mask = get_tissue_mask(img_thumb, tf_model)
    tf_model.release_model()

    mask_target_level = get_mask_target_level(slide, mask_downsamples)
    mask_ds = slide.level_downsamples[mask_target_level]
    mask_w = slide.level_dimensions[mask_target_level][0]
    mask_h = slide.level_dimensions[mask_target_level][1]

    mask = cv2.resize(thumb_mask, (mask_w, mask_h), interpolation=cv2.INTER_NEAREST)

    kernel = np.ones((15, 15), dtype=np.uint8)
    dialte_segmask = cv2.dilate(mask, kernel, iterations=1)
