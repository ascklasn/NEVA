...
def bootstrap_auc(y_true: np.ndarray | torch.Tensor | list, 
    ...
    y_true = to_numpy(y_true).astype(int)  # 修改这一行，确保y_true是整数类型
    y_prob = to_numpy(y_prob)
    
    assert len(np.unique(y_true)) >= 2, f"y_true must contain at least two unique classes for AUC computation. But get {np.unique(y_true)}"  # 至少要有两个类别，如果不满足则直接报错并输出‘y_true must contain at least two unique classes for AUC computation.’的错误信息
    assert y_prob.ndim == 2, "y_prob must be a 2D array for AUC computation."  # y_prob必须是二维数组
    # print(y_true, y_prob)
    assert y_true.shape[0] == y_prob.shape[0], f"y_true and y_prob must have the same number of samples.\n But get y_true:{y_true.shape},y_prob:{y_prob.shape}"  # y_true和y_prob的样本数必须相同
    ...
    auc_values = []
    target_class_set = set(existing_classes)

    while len(auc_values) < n_bootstrap:
        indices = np.random.choice(n_samples, size=n_samples, replace=True)
        sample_classes = set(np.unique(y_true[indices]))
        
        if sample_classes != target_class_set:
            continue  # 不包含完全相同的类别，跳过
    ...
    ci_low = np.percentile(auc_values, 100 * alpha / 2)
    ci_high = np.percentile(auc_values, 100 * (1 - alpha / 2))
    auc_mean = np.mean(auc_values)
    auc_std = np.std(auc_values)

    return auc_mean, ci_low, ci_high, auc_std, auc_values
...
def bootstrap_cindex(risks: np.ndarray | torch.Tensor | list,
                        times: np.ndarray | torch.Tensor | list, 
        ...
        Parameters:
            ...
            times (array-like): True survival times. Can be a numpy array, torch tensor, or list.
        ...
        status = to_numpy(status).astype(bool)  # 确保status是整数类型
        # 关键修改:将2D数组展平为1D数组
        if risks.ndim > 1:
            risks = risks.flatten()  # 或者 risks = risks.squeeze()
        if times.ndim > 1:
            times = times.flatten()  # 或者 pfs = pfs.squeeze()
        if status.ndim > 1:
            status = status.flatten()  # 或者 status = status.squeeze()
        ...
        for _ in range(n_bootstrap):
            ...
            if len(np.unique(status[indices])) > 1:
                cindex = concordance_index_censored(status[indices], times[indices], risks[indices])[0]
...
def compute_P_value(risks: np.ndarray | torch.Tensor | list, 
                    times: np.ndarray | torch.Tensor | list, 
    ...
    by risk level. The input `risks` are divided by their median value into high-risk and 
    low-risk groups. Then, a log-rank test is applied to compare the survival distributions of the two groups.

    Parameters:
        risks (array-like): Predicted continuous risk scores from a survival model (e.g., Cox model or neural network).
            - Shape: (n_samples,)
            - Type: NumPy array, PyTorch tensor, or list
        times (array-like): Observed survival times (time to event or censoring).
            - Shape: (n_samples,)
            - Type: NumPy array, PyTorch tensor, or list
    ...
    Notes:
        - The high-risk and low-risk groups are determined by the median value of `risks`.
    ...
    all_hazard_scores = to_numpy(risks)
    all_times = to_numpy(times)
    all_status = to_numpy(status)
    assert len(all_hazard_scores) == len(all_times) == len(all_status)
    ...
    # 按索引获取对应的数据
    high_risk_labels = all_times[high_risk_indices]
    high_risk_status = all_status[high_risk_indices]

    low_risk_labels = all_times[low_risk_indices]
...
# todo 绘制K-M曲线的函数,用来绘制K-M曲线，需要用到上面的compute_P_value函数(与HR(95%CI)函数合并)
